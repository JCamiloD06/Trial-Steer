...
el resumen de una corrida y los puntos de verificación de la plataforma. No
...
una, cuenta como hecha, haya completado o no la vuelta, porque una vuelta
fallida es un resultado y no se repite en silencio.
"""
import csv
import json
import math
...
def verificacion_plataforma(carpeta, cfg):
    """
    Puntos de verificación de la plataforma. Cada punto devuelve nombre, valor, criterio y si
    ...
    item("Física ampliada disponible", _fmt(meta.get("fisica_ampliada")),
         "sí, necesaria para medir ejes con contactos", bool(meta.get("fisica_ampliada")))
    ...
    item("Desalineación entre ejes y ψ", f"mediana {np.median(des):.2f} grados" if des.size else "sin dato",
         "pequeña, confirma el desfase de 90 grados", None)
    ...
    item("Velocidad al primer cruce", _fmt(dif, 1, " km/h"), f"|diferencia| menor a {tol} km/h",
    ...
    item("Ciclos con tiempo de cómputo bajo 50 ms", _fmt(None if pct50 is None else 100.0 - pct50, 2, " %"),
         "al menos 99 %", None if pct50 is None else (100.0 - pct50) >= 99.0)
    ...
    item("Saturaciones de magnitud y tasa",
         ...
         "informativo", None)
    item("Respaldo del controlador", _fmt(res.get("pct_respaldo_controlador"), 2, " %"), "informativo", None)
    ...
    item("Salidas de pista en la vuelta medida", str(eventos), "informativo", None)
    ...
    if m_r.sum() > 50:
        ...
        item("Constante de la cadena de dirección",
             ...
             "recalibrar si difiere más de 15 %", diferencia <= 15.0)
    else:
