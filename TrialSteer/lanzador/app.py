...
bloqueo del MPC longitudinal, añadida porque el original podía quedarse sin
acelerar en pista.
...
SCRIPT = RAIZ_APP / "ejecutar_corrida.py"
CONFIG_BASE = RAIZ_APP / "configs" / "base.json"
...
SCRIPT_GRAFICAS = procedencia.RAIZ_REPO / "scripts" / "graficas_corrida.py"
...
class Lanzador(tk.Tk):
    def __init__(self):
        ...
        # Tanda de sintonía. Encadena las corridas del plan de sintonía, con
        ...
        tab_ctrl = ttk.Frame(self.nb_principal)
        self.nb_principal.add(tab_ctrl, text="Prueba de controladores")
        tab_mpc = ttk.Frame(self.nb_principal)
        self.nb_principal.add(tab_mpc, text="MPC completo")

        self.nb = ttk.Notebook(tab_ctrl)
    ...
    def _tab_corrida(self):
        ...
        # plantilla, y esa acción toca la instalación del usuario.
    ...
    def _elegir_config(self):
        ruta = filedialog.askopenfilename(initialdir=str(RAIZ_APP / "configs"), filetypes=[("JSON", "*.json")])
        if ruta:
    ...
    def iniciar(self):
        ...
        self._iniciar_preparacion(reiniciar=False, destino="controladores")
    ...
    def usar_seleccionada(self):
        ...
        if fila["estado"] != "pendiente":
            if not messagebox.askyesno(
                    ...
                    "Ninguna vuelta se repite ni se descarta en silencio. "
    ...
    def iniciar_tanda_campana(self):
        ...
        if not messagebox.askyesno(
            ...
            return
        carpeta_log = procedencia.RAIZ_REPO / "datos" / "tandas"
    ...
    def _siguiente_de_tanda(self):
        ...
        self._iniciar_preparacion(reiniciar=True, destino="controladores")
    ...
    def _tab_verificacion(self):
        tab = ttk.Frame(self.nb)
        self.nb.add(tab, text="Verificación de la plataforma")
        barra = ttk.Frame(tab)
        barra.pack(fill="x", padx=6, pady=6)
    ...
    def evaluar(self):
        ...
        try:
            items = mod_corridas.verificacion_plataforma(self.mapa_verif[clave], self._cfg())
        except Exception as e:
    ...
    def iniciar_mpc(self):
        ...
        # Igual que en la corrida de un controlador, el juego se prepara siempre antes.
