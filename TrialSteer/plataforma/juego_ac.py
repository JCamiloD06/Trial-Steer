...
RUTA_CONFIG = procedencia.RAIZ_APP / "configs" / "juego_ac.json"
...
def cargar_config(ruta=RUTA_CONFIG):
    ...
    resolver_rutas(cfg)
    cfg["_dir_plantilla"] = procedencia.RAIZ_APP / cfg["plantilla"]
...
def juego_abierto():
    # errors="replace" porque tasklist escribe en la página de códigos de la consola
    # y en modo UTF-8 de Python su texto en español no se puede decodificar.
...
def _preparar(cfg, reiniciar, log, info):
    ...
    try:
        ...
        if not ok:
            ...
            # visto el 2026-09-16. Se espera a que el usuario pulse el volante.
