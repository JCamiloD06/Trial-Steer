...
Dos bloques que se corren en orden. El primero ajusta el perfil base y el
segundo mide la repetibilidad. El orden importa, porque
...
Bloque de repetibilidad. Las vueltas por controlador que fija
parametros_piloto.json, en perfil nominal, con el agarre ya fijado y todas
con la misma configuración. De ahí sale la desviación estándar del RMSE de
e_y entre vueltas idénticas.
...
RUTA_PARAMETROS = RAIZ_APP / "configs" / "parametros_piloto.json"
RUTA_PLAN = RAIZ_APP / "configs" / "plan_piloto.json"
DIR_CONFIGS = RAIZ_APP / "configs" / "piloto"
...
def anadir_bloque_tiempo_limite(plan, ruta=RUTA_PLAN, parametros=None):
    """
    Añade al plan el bloque del tiempo límite y lo reescribe.
    ...
    el plan guardó al generarse, porque esa copia es anterior al bloque. El
    plan registra la copia nueva, de modo que queda declarado con qué valores
    se creó el bloque.
    """
...
def escribir_config(corrida, config_base, dir_configs=DIR_CONFIGS):
    ...
    cfg["_descripcion"] = (f"Corrida {corrida['id_plan']} del plan del piloto. "
