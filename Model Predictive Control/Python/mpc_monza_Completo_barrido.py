...
class MPCLongitudinalController:
    ...
    def __init__(self, model_params, ts=0.05, horizon=10, v_scale_kmh=10.0,
        ...
        self.R = 0.5
        self.R_rate = 2.0
        # v_scale_kmh normaliza el error de velocidad dentro del coste.
        #
        # Sin esta escala el error va en km/h, de modo que con 10 km/h de
        # desvío el término de seguimiento vale unas cuatrocientas veces el
        # de esfuerzo y el óptimo es siempre pedal a fondo en un sentido o
        # en el otro. En la corrida del 2026-09-09 eso se ve directamente en
        # el comando crudo, que alterna entre menos uno y cero dentro de una
        # misma frenada, incluso soltando el freno por completo durante un
        # ciclo a 175 km/h. El conformador de pedal lo suaviza a la salida,
        # pero cada aplicar y soltar descarga el eje trasero, y con freno por
        # encima de 0.3 el deslizamiento trasero superaba al delantero en el
        # 54.7 por ciento de los ciclos.
        #
        # Dividiendo el error por esta escala los tres términos quedan
        # comparables en el margen normal de operación, así que el mando
        # modula. Con desvíos grandes el término de seguimiento vuelve a
        # dominar y el pedal llega al tope, que es lo que se quiere en una
        # frenada de verdad.
        self.v_scale = max(float(v_scale_kmh), 1e-3)
        self.u_prev = 0.0
        ...
        # sostenida mientras el error crece sin limite. Se detecta la racha
        # y se reinicia el punto de arranque con una estimacion de todo o
        # nada en la direccion correcta. No se toca mpc_monza_Completo.py.
        #
        # anti_bloqueo=False salta el bloque entero, con lo que el
        # comportamiento vuelve a ser el de la version previa a la
        # salvaguarda. Existe para poder medir la repetibilidad de ambas
        # condiciones desde una sola version del archivo.
        self.anti_bloqueo = bool(anti_bloqueo)
        self._stall_count = 0
        self._stall_start_err_kmh = None
    ...
    def cost_function(self, u_sequence, v0, v_ref):
        ...
        e = (v_pred[1:] - v_ref) / self.v_scale
        cost_tracking = np.sum(self.Q * e ** 2)
    ...
    def compute_control(self, current_speed_kmh, target_speed_kmh):
        ...
        self.last_nit = int(getattr(res, "nit", -1))
        self.last_cost = float(res.fun) if np.isfinite(res.fun) else float("nan")
        # Qué se aplica cuando el optimizador no declara convergencia.
        #
        # La versión anterior aplicaba np.full(N, -0.3), un 30 por ciento de
        # freno fijo decidido por el fallo del solver y no por el estado del
        # vehículo. En la corrida del 2026-09-09 eso ocurrió en el 2.87 por
        # ciento de los ciclos y subió al 7.6 por ciento en la ventana del
        # despiste, y en el 8.8 por ciento de esos casos el vehículo iba por
        # debajo del objetivo, o sea que frenaba cuando debía acelerar.
        #
        # Ahora se usa el último iterado. L-BFGS-B arranca en el warm start y
        # es un método de descenso, así que su iterado final nunca tiene coste
        # mayor que el warm start del que partió. No converger casi siempre
        # significa que agotó iteraciones o que la búsqueda lineal se estancó,
        # no que el resultado sea inservible. Solo si el iterado no es
        # utilizable se recurre al plan anterior.
        u_seq = res.x
        if (not np.all(np.isfinite(u_seq))) or u_seq.shape != (self.N,):
            u_seq = self._u_warm
        u_optimal = float(np.clip(u_seq[0], -1.0, 1.0))
        self._u_warm = np.concatenate([u_seq[1:], [u_seq[-1]]])
        self.u_prev = u_optimal

        # --- Salvaguarda anti bloqueo (ver __init__ para el porque) ---
        if not self.anti_bloqueo:
            return u_optimal
        STALL_CICLOS = 8
        STALL_TOL_KMH = 0.5
        err_abs_kmh = abs(current_speed_kmh - target_speed_kmh)
        if not self.last_ok:
            if self._stall_count == 0:
                self._stall_start_err_kmh = err_abs_kmh
            self._stall_count += 1
        else:
            self._stall_count = 0
            self._stall_start_err_kmh = None

        if (self._stall_count >= STALL_CICLOS and self._stall_start_err_kmh is not None
                and err_abs_kmh >= self._stall_start_err_kmh - STALL_TOL_KMH):
            seed = 1.0 if target_speed_kmh > current_speed_kmh else -1.0
            print(f"      [ANTI-BLOQUEO] {self._stall_count} ciclos sin converger, "
                  f"error {err_abs_kmh:.1f} km/h sin mejorar. Reinicio warm start a "
                  f"{seed:+.0f}.")
            self._u_warm = np.full(self.N, seed)
            self._stall_count = 0
            self._stall_start_err_kmh = None
...
class PIDLongitudinalController:
    def __init__(self, kp=0.06, ki=0.015, kd=0.01, dt=0.05):
        self.kp, self.ki, self.kd = kp, ki, kd
        self.dt = dt
        self.integral = 0.0
        self.prev_error = 0.0
        # Para que el bucle principal pueda tratarlo igual que
        # MPCLongitudinalController al registrar la corrida.
        self.last_ok = True
        self.last_nit = 0
        self.last_cost = 0.0
        self.u_prev = 0.0
    ...
    def compute_control(self, current_speed_kmh, target_speed_kmh):
        ...
        self.integral = float(np.clip(self.integral + error * self.dt, -50.0, 50.0))
...
class RunRecorder:
    ...
    def _stats(self):
        ...
        try:
            ...
            # Se descartan además las muestras con deriva apreciable. Un
            # trompo o un despiste ocurre justo en el rango de velocidad que
            # esta identificación usa, y la relación cinemática no vale ahí.
            # En la corrida del 2026-09-09 no filtrar bajó el ajuste de 0.4906
            # con R² de 0.984 a 0.4249 con R² de 0.921, un sesgo del 13 por
            # ciento en la constante que gobierna toda la cadena.
            v_ref_alto = v_real > 60
            if int(v_ref_alto.sum()) >= 50:
                off_conv = float(np.median(sside[v_ref_alto]))
            else:
                off_conv = 0.0
            deriva = np.abs((sside - off_conv + np.pi) % (2 * np.pi) - np.pi)
            m = ((fuera == 0) & (np.abs(ey_all) < 2.0) & (vk > 20) & (vk < 50)
                 & (np.abs(yaw) > 0.03) & (np.abs(ac) > 0.02)
                 & (deriva < math.radians(3.0)))
            if int(m.sum()) >= 40:
...
CFG = {
    ...
    "long_v_scale_kmh": 10.0,      # escala del error de velocidad dentro del coste.
                                   # Sin ella el mando resulta de todo o nada
    ...
    #
    # Valor vigente medido en la tercera corrida, ya con el vehículo trazando
    # bien y por tanto en mejores condiciones de medida. Descartando la
    # ventana del despiste da 0.4315 con R² de 0.984, frente a 0.3737 con R²
    # de 0.921 incluyéndola. Son 24.6 grados de rueda con el eje a fondo.
    "rueda_rad_eje_completo": 0.43,
    "theta_clip_rad": 1.2,
    "vjoy_max_rad": 1.2,
}


def main(use_ffbeast=False, overrides=None, arch="coordinado", run_label=None,
         anti_bloqueo=True, run_subdir=None):
    # Se descartan aqui las llaves en None (argumento no pasado en la CLI).
    # Sin este filtro, overrides.get('throttle_tau', 17.20) mas abajo
    # devuelve None en vez del default, porque la llave SI existe (con
    ...
    if use_ffbeast:
        ...
        if "J_volante" in overrides or "b_volante" in overrides:
            print(f"      [OVERRIDE] planta simulada real: J={J_volante_real} "
                  f"b={b_volante_real}  (el MPC sigue creyendo J=0.02, b=0.1)")
        plant = SimulatedSteeringPlant(Ts=0.05, J=J_volante_real, b=b_volante_real)
    ...
    model_params = {
        # Throttle: identificado el 2026-09-09 sobre la tercera corrida, con
        # 4208 muestras de acelerador a fondo, sin freno y con las cuatro
        # ruedas en pista, ajustando la recta dv/dt igual a K entre tau menos
        # v entre tau.
        #
        # Sustituye a los valores anteriores, K de 122.8 y tau de 2.81 s, que
        # venían de pruebas en marcha 2 contra el limitador de revoluciones y
        # estaban documentados como provisionales. No solo eran imprecisos,
        # tenían el SIGNO cambiado en el rango de trabajo. El modelo antiguo
        # predecía menos 17.84 km/h por segundo a 150-185 km/h con el pedal a
        # fondo, cuando el vehículo acelera a más 4.64. El 45 por ciento de la
        # corrida transcurre por encima de los 122.8 km/h del K antiguo, o sea
        # en la zona donde el modelo creía que acelerar frenaba, y por eso el
        # optimizador llegaba a pedir gas en plena frenada.
        #
        # Contraste del modelo nuevo contra lo observado, en km/h por segundo.
        #    40-80 km/h   observado +10.89   modelo +10.43
        #    80-120 km/h  observado  +9.93   modelo  +8.74
        #   120-150 km/h  observado  +6.78   modelo  +6.73
        #   150-185 km/h  observado  +4.64   modelo  +4.41
        #
        # El K identificado, 248.7 km/h, concuerda dentro del 3 por ciento con
        ...
        # overrides["brake_a_max_ms2"] cambia solo la creencia del MPC, no
        # la capacidad real de frenado del vehiculo en AC.
    ...
    if arch == "lateral_pid":
        ...
        mpc_speed = MPCLongitudinalController(model_params, ts=0.05, horizon=10,
                                              ...
                                              anti_bloqueo=anti_bloqueo)
        if not anti_bloqueo:
            print("      [SALVAGUARDA] anti bloqueo DESACTIVADA para esta corrida")
        mpc_speed.Q = CFG["long_Q"]
        mpc_speed.R = CFG["long_R"]
        mpc_speed.R_rate = CFG["long_R_rate"]
    else:
        raise ValueError(f"--arch desconocido: {arch!r} (use 'coordinado' o 'lateral_pid')")
    ...
    cfg_corrida.update({
        ...
        "long_horizon": getattr(mpc_speed, "N", None),
        "long_ts": getattr(mpc_speed, "ts", None),
        "modelo_longitudinal": model_params,
        "planta_direccion": "FFBeast" if use_ffbeast else "simulada",
        "ay_max_ms2": 6.5, "ax_brake_max_ms2": 10.25,
        # Trazabilidad de la prueba de robustez/comparacion (BARRIDO):
        "barrido_arquitectura_longitudinal": arch,
        "barrido_run_label": run_label,
        "barrido_overrides": overrides,
        "barrido_anti_bloqueo": bool(anti_bloqueo),
        "barrido_run_subdir": run_subdir,
        "barrido_J_volante_real": J_volante_real if not use_ffbeast else None,
        "barrido_b_volante_real": b_volante_real if not use_ffbeast else None,
    })
    runs_base = os.path.join(os.path.dirname(os.path.abspath(__file__)), "runs")
    if run_subdir:
        # Subcarpeta por prueba (5.1_incertidumbre, 5.2_montecarlo, etc.),
        # solo para organizar. El analisis no depende de esto, ya filtra por
        # barrido_run_label dentro del manifiesto, no por ubicacion en disco.
        runs_base = os.path.join(runs_base, run_subdir)
...
def _parse_args():
    ...
    p.add_argument("--sin-salvaguarda", action="store_true",
                    ...
                    help="desactiva la salvaguarda anti bloqueo del MPC "
                         "longitudinal, devolviendo el comportamiento previo "
                         "a ella. Sirve para medir la repetibilidad de esa "
                         "condicion desde esta misma version del archivo.")
    p.add_argument("--run-subdir", default=None, dest="run_subdir",
    ...
    p.add_argument("--rueda-rad-eje-completo", type=float, default=None,
                    dest="rueda_rad_eje_completo",
                    help="lo que el generador de referencia CREE que vale la "
                         "cadena de direccion. La conversion real vJoy->rueda "
                         "en AC no cambia con esto.")
    p.add_argument("--grip-usage-factor", type=float, default=None,
                    dest="grip_usage_factor",
                    help="margen de agarre usado SOLO para planificar el "
                         "perfil de velocidad. El agarre real de los "
                         "neumaticos en AC no cambia con esto.")
    p.add_argument("--throttle-K", type=float, default=None, dest="throttle_K")
...
if __name__ == "__main__":
    args = _parse_args()
    _overrides = {
        "k_ey": args.k_ey, "ff_gain": args.ff_gain,
        "rueda_rad_eje_completo": args.rueda_rad_eje_completo,
        "grip_usage_factor": args.grip_usage_factor,
        "throttle_K": args.throttle_K, "throttle_tau": args.throttle_tau,
        "brake_a_max_ms2": args.brake_a_max_ms2,
        "J_volante": args.J_volante, "b_volante": args.b_volante,
    }
    if args.ffbeast and (args.J_volante is not None or args.b_volante is not None):
        print("ADVERTENCIA: --J-volante/--b-volante no tienen efecto con "
              "--ffbeast (esos overrides son solo de la planta simulada). "
              "Se ignoran.")
    main(use_ffbeast=args.ffbeast, overrides=_overrides, arch=args.arch,
         run_label=args.run_label, anti_bloqueo=not args.sin_salvaguarda,
